...
const VendorDashboard = () => {
  ...
  const { runningJobs } = useGenerationWorker(() => {
    fetchProducts();
  });
  ...
  return (
    <div className="h-screen overflow-hidden bg-background dark:bg-black/20 flex text-left print:h-auto print:overflow-visible print:bg-white print:text-black">
              ...
              >
                ...
                {activeTab === 'inventory' && (
                  <ProductManagement
                    ...
                    showImportButton={hasActiveSpecialQuota}
                  />
                )}
                {activeTab === 'orders' && <OrderManagement />}
                {activeTab === 'guarantee' && <GuaranteePayment />}
                {activeTab === 'messages' && <VendorMessages />}
                {activeTab === 'quota' && <VendorQuota setShowImportModal={setShowImportModal} canShowImportButton={hasActiveSpecialQuota} />}
      ...
      <div className="print:hidden">
        <GenerationProgress runningJobs={runningJobs} />
      </div>
