import { ArrowLeft, Smartphone, LogOut, Loader2, Globe, Clock, Trash2, ShieldCheck } from 'lucide-react';
...
const LoginHistory = () => {
  ...
  const { sessions, revoking, handleRevokeOthers, handleRevokeSingle } = useSecurity();
      ...
      <section className="bg-white dark:bg-slate-900 rounded-[32px] p-8 shadow-xl border border-slate-200 dark:border-slate-800 space-y-8">
              ...
              <div className="flex items-center gap-5">
                ...
                <div>
                  <div className="flex items-center gap-2">
                    ...
                    {session.current && (
                      <span className="px-2 py-0.5 bg-green-500 text-white text-[7px] font-black uppercase tracking-widest rounded-full">{t('login_history_current')}</span>
                    )}
                  </div>
        ...
        <div className="pt-2 flex justify-center">
          <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest flex items-center gap-2">
            <ShieldCheck size={12} /> {t('login_history_last_security')} Today at 01:48 AM
          </p>
        </div>
      </section>
    </div>
