...
import {
  ...
  Loader2, CircleDollarSign, ChevronDown
} from 'lucide-react';
...
/* ─── Types ────────────────────────────────────────────────────── */
const SHOP_WALLET_KEY = 'snapbuy_dashboard_shop_id';
...
/* ─── Shop avatar (logo w/ fallback) ─────────────────────────── */
const ShopAvatar = ({ shop: s, size = 36 }: { shop: any; size?: number }) => (
  s?.logo_url ? (
    <img
      src={s.logo_url}
      alt={s?.name || 'shop'}
      style={{ width: size, height: size }}
      className="rounded-xl object-cover shrink-0 border border-slate-200 dark:border-slate-700 bg-white"
    />
  ) : (
    <div
      style={{ width: size, height: size }}
      className="rounded-xl bg-primary-500/10 flex items-center justify-center shrink-0 border border-slate-200 dark:border-slate-700"
    >
      <Store size={Math.round(size * 0.45)} className="text-primary-500" />
    </div>
  )
);
...
const UserDashboard = () => {
  ...
  // ─── Shop wallet state (selection persisted in localStorage) ───
  const [selectedShopId, setSelectedShopId] = useState<string>(() => localStorage.getItem(SHOP_WALLET_KEY) || '');
  const [shopWallet, setShopWallet] = useState<any>(null);
  ...
  // Restore saved selection (or default) once shops are loaded
  useEffect(() => {
    if (shops && shops.length > 0) {
      const saved = localStorage.getItem(SHOP_WALLET_KEY);
      if (!selectedShopId) {
        const exists = saved && shops.some((s: any) => s.id === saved);
        setSelectedShopId(exists ? saved! : (shop?.id || shops[0].id));
      } else if (!shops.some((s: any) => s.id === selectedShopId)) {
        setSelectedShopId(shop?.id || shops[0].id);
      }
    }
  }, [shops, shop, selectedShopId]);

  // Persist selection so the next visit opens the same shop
  useEffect(() => {
    if (selectedShopId) localStorage.setItem(SHOP_WALLET_KEY, selectedShopId);
  }, [selectedShopId]);

  // Fresh shop wallet balances for the selected shop
  const fetchShopWallet = useCallback(async () => {
    if (!selectedShopId) { setShopWallet(null); return; }
    const { data } = await supabase
      .from('shops')
      .select('id, name, sale_balance, bonus_balance')
      .eq('id', selectedShopId)
      .single();
    setShopWallet(data || null);
  }, [selectedShopId]);

  useEffect(() => { fetchShopWallet(); }, [fetchShopWallet]);
  ...
  const fetchDashboardData = useCallback(async () => {
    try {
      ...
      if (shop?.id) {
        const [prodRes, ordersRes, promoRes] = await Promise.all([
          (async () => supabase.from('products').select('id,is_published').eq('shop_id', shop.id))(),
          (async () => supabase.from('orders').select('id,total_amount,status', { count: 'exact' }).eq('shop_id', shop.id).order('created_at', { ascending: false }).limit(5))(),
          // Live DB has no product_boosts table — active promotions live on products.is_promoted
          (async () => supabase.from('products').select('id', { count: 'exact', head: true }).eq('shop_id', shop.id).eq('is_promoted', true).gt('promoted_until', new Date().toISOString()))(),
        ]);
        products = prodRes?.data || [];
        orders = ordersRes?.data || [];
        totalOrderCount = ordersRes?.count || 0;
        activeBoosts = promoRes?.count || 0;
      }
  ...
  const handleRefresh = async () => {
    ...
    await fetchShopWallet();
  };

  // ─── Shop wallet transfer (shop → account wallet) ──────────────
  const selectedShop = shops?.find((s: any) => s.id === selectedShopId) || null;
  const shopWalletTotal = Number(shopWallet?.sale_balance || 0) + Number(shopWallet?.bonus_balance || 0);
  const transferVal = parseFloat(transferAmount) || 0;
  const canTransfer = transferVal > 0 && transferVal <= shopWalletTotal;

  const handleTransfer = async () => {
    if (!shopWallet || !canTransfer) return;
    setTransferring(true);
    try {
      const { data, error } = await supabase.rpc('withdraw_shop_wallet', {
        p_shop_id: shopWallet.id,
        p_amount: transferVal,
      });
      if (error) throw error;
      toast.success(`✅ ${t('user_dashboard_transfer_success', 'Transferred')} ${transferVal.toLocaleString('en-US', { style: 'currency', currency: 'USD' })}`);
      setTransferAmount('');
      await Promise.all([
        fetchShopWallet(),
  ...
  return (
    <div className="w-full max-w-5xl mx-auto min-h-screen bg-background pb-32 animate-fade-in text-left">
      ...
      <div className="p-4 sm:p-6 space-y-8">
        ...
        {/* ─── Shop Wallet (per-shop balance + transfer to account) ─── */}
        {shops && shops.length > 0 && (
          <section>
            <h2 className="text-xs font-black uppercase tracking-widest text-slate-500 mb-3 flex items-center gap-2">
              <Store size={14} className="text-primary-500" /> {t('user_dashboard_shop_wallet', 'Shop Wallet')}
            </h2>
            <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 p-4 sm:p-5 space-y-4 sm:space-y-5">
              {/* Shop selector — logo + balance, remembered in localStorage */}
              <div ref={shopDropdownRef} className="relative">
                <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest">{t('user_dashboard_select_shop', 'Select Shop')}</label>
                <button
                  type="button"
                  onClick={() => setShopDropdownOpen(o => !o)}
                  className="mt-1.5 w-full flex items-center gap-3 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl px-3 py-2.5 text-left hover:border-primary-500/50 focus:outline-none focus:ring-2 focus:ring-primary-500/50 transition-all"
                >
                  <ShopAvatar shop={selectedShop} size={36} />
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-black text-slate-900 dark:text-white truncate">{selectedShop?.name || '—'}</p>
                    <p className="text-[10px] font-black text-primary-500">${shopWalletTotal.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</p>
                  </div>
                  <ChevronDown size={16} className={`text-slate-400 transition-transform shrink-0 ${shopDropdownOpen ? 'rotate-180' : ''}`} />
                </button>
                ...
                {shopDropdownOpen && (
                  <div className="absolute z-30 mt-2 w-full bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-2xl shadow-2xl overflow-hidden max-h-72 overflow-y-auto">
                    {shops.map((s: any) => {
                      const sTotal = Number(s.sale_balance || 0) + Number(s.bonus_balance || 0);
                      ...
                      return (
                        <button
                          ...
                          className={`w-full flex items-center gap-3 px-3 py-2.5 text-left transition-colors ${active ? 'bg-primary-500/10' : 'hover:bg-slate-50 dark:hover:bg-slate-800'} ${active ? '' : 'border-b border-slate-100 dark:border-slate-800/60 last:border-b-0'}`}
                        >
                          <ShopAvatar shop={s} size={36} />
                          <div className="flex-1 min-w-0">
                            <p className="text-sm font-black text-slate-900 dark:text-white truncate">{s.name}</p>
                            <p className="text-[9px] font-black uppercase tracking-widest text-slate-400">{t('user_dashboard_shop_balance', 'Shop Balance')}</p>
                          </div>
                          <div className="flex items-center gap-2 shrink-0">
                            <span className={`text-sm font-black ${active ? 'text-primary-500' : 'text-slate-600 dark:text-slate-300'}`}>
                              ${sTotal.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                            </span>
                            {active && <CheckCircle2 size={16} className="text-primary-500 shrink-0" />}
                          </div>
                        </button>
                      );
                    })}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div className="bg-primary-500/5 border border-primary-500/20 rounded-xl p-3">
                  <p className="text-[9px] font-black uppercase tracking-widest text-slate-500 mb-1 flex items-center gap-1.5">
                    <Store size={11} className="text-primary-500" /> {t('user_dashboard_shop_balance', 'Shop Balance')}
                  </p>
                  <p className="text-base sm:text-lg font-black text-primary-600 dark:text-primary-400 truncate" title={`$${shopWalletTotal.toFixed(2)}`}>
                    ${shopWalletTotal.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                  </p>
                </div>
                <div className="bg-green-500/5 border border-green-500/20 rounded-xl p-3">
                  <p className="text-[9px] font-black uppercase tracking-widest text-slate-500 mb-1 flex items-center gap-1.5">
                    <Wallet size={11} className="text-green-500" /> {t('user_dashboard_personal_balance', 'Account Balance')}
                  </p>
                  <p className="text-base sm:text-lg font-black text-green-600 dark:text-green-400 truncate" title={`$${(profile?.wallet_balance || 0).toFixed(2)}`}>
                    ${(profile?.wallet_balance || 0).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                  </p>
                </div>
              </div>

              {/* Transfer to account wallet (same flow as vendor wallet page) */}
              <div className="space-y-2">
                <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest">{t('user_dashboard_transfer_amount', 'Transfer to Account Wallet')}</label>
                <div className="relative">
                  ...
                  <input
                    ...
                    placeholder="0.00"
                    className="w-full pl-8 pr-4 py-3 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl text-base sm:text-sm font-bold text-slate-900 dark:text-white placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-primary-500/50 focus:border-primary-500 transition-all"
                  />
                </div>
                <div className="grid grid-cols-4 gap-2">
                  {[25, 50, 75, 100].map(pct => (
                    <button
                      key={pct}
                      type="button"
                      onClick={() => setTransferAmount(((shopWalletTotal * pct) / 100).toFixed(2))}
                      className="py-2 rounded-lg bg-slate-100 dark:bg-slate-800 hover:bg-primary-500/10 hover:text-primary-500 active:scale-95 text-[11px] font-black text-slate-500 transition-all border border-slate-200 dark:border-slate-700"
                    >
                      {pct}%
                    </button>
                  ))}
                </div>
                {transferVal > shopWalletTotal && (
                  <p className="text-[10px] font-bold text-red-500">⚠ {t('user_dashboard_insufficient_shop', 'Insufficient shop wallet balance')}</p>
                )}
              </div>
              ...
              <button
                ...
                className="w-full py-3 bg-primary-500 hover:bg-primary-600 text-white rounded-xl font-black uppercase tracking-widest text-xs transition-all disabled:opacity-40 disabled:cursor-not-allowed flex items-center justify-center gap-2 shadow-lg shadow-primary-500/20"
              >
                {transferring
                  ? <><Loader2 size={15} className="animate-spin" />...</>
                  : <><CircleDollarSign size={15} /> {t('user_dashboard_transfer_btn', 'Transfer')}{transferVal > 0 ? ` $${transferVal.toFixed(2)}` : ''}</>}
              </button>
            </div>
          </section>
        )}
                    ...
                    <div className="divide-y divide-slate-100 dark:divide-slate-800">
                      {(data?.recentOrders || []).map((order: any) => (
                        <div key={order.id} className="px-5 py-3 flex items-center justify-between gap-4">
                          ...
                          <p className="text-[11px] font-black text-slate-800 dark:text-white shrink-0">${(order.total_amount || 0).toFixed(2)}</p>
                        </div>
                      ))}
                    </div>
                  )}
                ...
                return (
                  <div key={s.id} className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 px-5 py-4 flex items-center gap-4">
                    ...
                    <div className="text-right shrink-0">
                      <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">{t('user_dashboard_shop_wallet', 'Shop Wallet')}</p>
                      <p className="text-sm font-black text-primary-500">${(Number(s.sale_balance || 0) + Number(s.bonus_balance || 0)).toFixed(2)}</p>
                    </div>
                  </div>
                );
              })}
