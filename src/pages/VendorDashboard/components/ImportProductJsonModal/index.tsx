      ...
      >
        ...
        {totalLimit > 0 && (
          <div className={`px-6 py-3 flex items-center gap-3 border-b flex-shrink-0 ${isQuotaBlocked
            ...
            <div className={`w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0 ${isQuotaBlocked ? 'bg-amber-100 dark:bg-amber-900/40' :
              exceedsApiQuota ? 'bg-blue-100 dark:bg-blue-900/40' : 'bg-emerald-100 dark:bg-emerald-900/40'
              }`}>
              {isQuotaBlocked
            ...
            <div className="flex-1 min-w-0">
              <p className={`text-[10px] font-black uppercase tracking-widest ${isQuotaBlocked ? 'text-amber-600 dark:text-amber-400' :
                exceedsApiQuota ? 'text-blue-600 dark:text-blue-400' : 'text-emerald-600 dark:text-emerald-400'
                }`}>
                {isQuotaBlocked
                  ? 'พักชั่วคราว (รอโควต้ารูปภาพรีเซ็ต)'
                  : exceedsApiQuota
                    ? 'โควต้าไม่พอ (ระบบจะพักและรันต่ออัตโนมัติ)'
                    : t('vendor_import_api_ready')}
              </p>
              <p className="text-[10px] text-slate-500 dark:text-slate-400 mt-0.5">
                {isQuotaBlocked
                  ? (cooldownMins ? `กำลังพักชั่วคราว จะรันต่อใน ${cooldownMins} นาที` : 'กำลังพักชั่วคราว')
                  : t('vendor_import_api_can_import', { count: totalRemaining, remaining: totalRemaining, total: totalLimit })}
              </p>
            </div>
            <div className="flex-shrink-0 text-right">
              <span className={`text-lg font-black tabular-nums ${isQuotaBlocked ? 'text-amber-500' : exceedsApiQuota ? 'text-blue-500' : 'text-emerald-500'
                }`}>{totalRemaining}</span>
              ...
              <div className="flex items-center gap-2">
                ...
                <button
                  ...
                  onClick={() => {
                    ...
                    setGenerateCount(max);
                    setGenerateCountInput(String(max));
                  }}
                  className="flex-shrink-0 px-3 py-2.5 rounded-xl border-2 border-primary-500 bg-primary-500/10 text-primary-600 dark:text-primary-400 text-[10px] font-black uppercase tracking-widest hover:bg-primary-500 hover:text-white transition-all"
                >
                  MAX
                </button>
              </div>
        ...
        <div className="border-t border-slate-100 dark:border-slate-800/60 bg-slate-50 dark:bg-slate-900/80 px-6 py-4 flex-shrink-0">
          ...
          <div className="flex gap-3">
            ...
            <button
              ...
              disabled={isGenerating || selectedCategoryIds.length === 0 || generateCount < 1 || totalToGenerate > availableSlots}
