...
import { Settings2, Save, Loader2, CheckCircle2, RefreshCcw, Key, Plus, Trash2, Power } from 'lucide-react';
...
export const SiteSettings: React.FC = () => {
  ...
  const settingsMeta: Record<string, { label: string; description: string; prefix?: string; suffix?: string; icon: React.ReactNode }> = {
    // token_exchange_rate_usd_per_token removed from UI
  };
    ...
    <div className="space-y-6">
      ...
      {loading ? (
        ...
        <div className="grid gap-5">
          {settings.filter(s => s.key !== 'token_exchange_rate_usd_per_token').map((setting, i) => {
