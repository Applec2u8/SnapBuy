...
export const useVendorDashboard = () => {
  ...
  const [activeTab, _setActiveTab] = useState(() => {
    ...
    return ['overview', 'inventory', 'orders', 'messages', 'settings'].includes(hash) ? hash : 'overview';
  });
  ...
  useEffect(() => {
    const handleHashChange = () => {
      const hash = window.location.hash.replace('#', '');
      if (['overview', 'inventory', 'orders', 'messages', 'settings'].includes(hash)) {
        setActiveTab(hash);
      }
    };
  ...
  const fetchProducts = async (params?: {
    ...
    try {
      ...
      if (error) throw error;
      console.log('[VendorDashboard] fetched products sample:', (data || []).slice(0, 3).map((item: any) => ({ id: item.id, images: item.images, variants: item.product_variants?.length })));
      setProducts(data || []);
