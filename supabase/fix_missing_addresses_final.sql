-- 1. Create a correct RLS policy so Vendors can see the addresses
-- First drop the old one if it exists (just in case)
DROP POLICY IF EXISTS "addr_vendor_view" ON user_addresses;

-- Create the correct policy (orders links to shop through order_items)
CREATE POLICY "addr_vendor_view" ON user_addresses FOR SELECT USING (
  id IN (
    SELECT o.shipping_address_id FROM orders o
    JOIN order_items oi ON o.id = oi.order_id
    WHERE oi.shop_id IN (SELECT id FROM shops WHERE owner_id = auth.uid())
      AND o.shipping_address_id IS NOT NULL
  )
);

-- 2. Ensure all bots have an address
INSERT INTO public.user_addresses (
  user_id, full_name, phone, province, city, district, postal_code, address_line, is_default
)
SELECT 
  p.id, 
  p.full_name, 
  '08' || lpad(floor(random() * 100000000)::text, 8, '0'),
  'Bangkok', 'Bangkok', 'Pathum Wan', '10330', 
...
-- 3. Ensure all orders have shipping_address_id linked to the bot's address
...
-- 4. Update run_bot_job to correctly create a random address if missing
