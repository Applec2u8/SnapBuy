...
export const useShop = () => {
  ...
  }, [searchTerm, selectedCategories, selectedShops, shopSearch, sortBy, minPrice, maxPrice]);
  ...
  const fetchProducts = async (pageIndex: number, isInitial: boolean) => {
    try {
      ...
      let query = supabase
        .from('products')
        .select('*, shops(name, logo_url), categories(name)', { count: 'exact' })
        .eq('is_published', true);
      ...
      if (shopSearch.trim()) {
        // filter by shop name via a subquery-like approach: get shop ids matching shopSearch first
        const { data: matchedShops } = await supabase
          .from('shops')
          .select('id')
          .ilike('name', `%${shopSearch.trim()}%`);
        const matchedIds = (matchedShops || []).map((s: any) => s.id);
        if (matchedIds.length > 0) {
          query = query.in('shop_id', matchedIds);
        } else {
          // No shop matches - return empty
          setProducts([]);
          setHasMore(false);
          setLoading(false);
