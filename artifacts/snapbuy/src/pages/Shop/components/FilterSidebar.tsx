              ...
              <div className="space-y-2 pt-3 border-t border-slate-200 dark:border-slate-800">
                ...
                <div className="grid grid-cols-3 gap-2">
                  {shops.map(shop => (
                    ...
                    >
                      <img
                        src={shop.logo_url || shop.image_url || 'https://via.placeholder.com/60'}
                        alt={shop.name}
                        className="w-full h-full object-cover aspect-square"
                      />
                      <div className="absolute inset-0 bg-black/0 group-hover:bg-black/20 transition-colors" />
                      {selectedShops.includes(shop.id) && (
                        <div className="absolute inset-0 flex items-center justify-center bg-primary-500/90">
                          <Check size={16} className="text-white" />
                        </div>
                      )}
                      <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/40 to-transparent px-1 py-1">
                        <p className="text-[8px] font-bold text-white truncate text-center">{shop.name}</p>
                      </div>
                    </button>
                  ))}
                </div>
