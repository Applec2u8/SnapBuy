...
export const PurchaseModal: React.FC<PurchaseModalProps> = ({ product, isOpen, onClose }) => {
  ...
  const handlePurchase = async () => {
    ...
    let currentAddressId = addresses[0]?.id;
    if (!currentAddressId) {
      try {
        const { data: newAddr, error: addrErr } = await supabase
          ...
          .insert([{
            user_id: user.id,
            full_name: user?.email?.split('@')[0] || 'Admin Tester',
            phone: '0800000000',
            province: 'Test Province',
            city: 'Test City',
            district: 'Test District',
            postal_code: '10000',
            address_line: '123 Test Street (Auto-generated)',
            is_default: true
          }])
          .select()
          .single();
        ...
        toast.error('Please add a shipping address first (Auto-create failed)');
        return;
      }
    }
    ...
    try {
      ...
      const { data, error } = await supabase.rpc('place_order', {
        ...
        p_shipping_address_id: currentAddressId,
