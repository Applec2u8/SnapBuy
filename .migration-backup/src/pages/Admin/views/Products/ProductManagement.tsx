...
export const ProductManagement: React.FC<ProductManagementProps> = ({
  ...
  // const handleDeleteAllForShop = async (targetShopId: string) => {
  //   if (!targetShopId) return;
  //   if (!window.confirm('WARNING: Are you absolutely sure you want to delete ALL products for this shop? This action is irreversible.')) return;
  //   setIsDeleting(true);
  //   try {
  //     const { error } = await supabase.from('products').delete().eq('shop_id', targetShopId);
  //     if (error) throw error;
  //     setSelectedIds([]);
  //     // Re-fetch
  //     onFetch({ page: 1, pageSize: itemsPerPage, search: searchTerm, category: filterCategory, sortBy, shopId: selectedShopFilter || shopId });
  //   } catch (err: any) {
  //     console.error('Failed to delete all products for shop:', err);
  //     alert('Failed to delete all products for shop: ' + err.message);
  //   } finally {
  //     setIsDeleting(false);
  //   }
  // };
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white dark:bg-slate-900 p-4 sm:p-5 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-sm space-y-4">
        <div className="relative group">
          ...
          />
          {/* Inline loading spinner — right side of search bar */}
          {isFetching && (
      ...
      {/* Bulk Action Bar */}
      {(selectedIds.length > 0 || selectedShopFilter || shopId) && (
        <div className="flex flex-col sm:flex-row items-center justify-between bg-slate-50 dark:bg-slate-800/50 p-4 rounded-2xl border border-slate-200 dark:border-slate-700 gap-4">
          <div className="flex items-center gap-4">
            <button
              onClick={() => {
                const currentIds = products.filter(p => p.is_published).map(p => p.id);
              ...
              className="text-[10px] font-black uppercase tracking-widest text-slate-500 hover:text-primary-500 transition-colors"
            >
              {products.filter(p => p.is_published).every(p => selectedIds.includes(p.id)) && products.filter(p => p.is_published).length > 0
                ? 'Deselect Page' : 'Select Page'}
            </button>
            {selectedIds.length > 0 && (
              <span className="text-[10px] font-bold text-primary-500 bg-primary-50 dark:bg-primary-500/10 px-2 py-1 rounded-md">
          ...
          <div className="flex items-center gap-3">
            {selectedIds.length > 0 && (
              <button
                ...
                disabled={isDeleting}
                className="px-4 py-2 bg-red-500 hover:bg-red-600 text-white rounded-xl text-[10px] font-black uppercase tracking-widest transition-colors disabled:opacity-50"
              >
            ...
            {(selectedShopFilter || shopId) && (
              <button
                onClick={() => {
                  // open selection modal so admin can pick which products to clear
                  setSelectedIds([]);
                  setShowSelectionModal(true);
                }}
                disabled={isDeleting}
                className="px-4 py-2 border-2 border-red-500 text-red-500 hover:bg-red-50 dark:hover:bg-red-500/10 rounded-xl text-[10px] font-black uppercase tracking-widest transition-colors disabled:opacity-50"
              >
                Clear All Products
              </button>
            )}
      ...
      {isFetching && products.length === 0 ? (
        ...
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-4">
          {products
            ...
            ${isSelected ? 'border-primary-500 bg-primary-50 dark:bg-primary-900/10' : editingId === product.id ? 'border-primary-500 bg-primary-500/5' : 'border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-sm hover:shadow-lg hover:border-slate-200 dark:hover:border-slate-700'}
          `}>
                  <div
                    onClick={() => {
                      // toggle selection when clicking anywhere on the card
                      ...
                      <div className="relative w-20 h-20 sm:w-24 sm:h-24 rounded-2xl overflow-hidden flex-shrink-0 group/img border border-slate-200 dark:border-slate-800">
                        ...
                        <div className="absolute top-1.5 left-1.5 z-20">
                          <input
                            ...
                            onClick={(e) => e.stopPropagation()}
                            className="w-5 h-5 rounded border-slate-300 dark:border-slate-600 text-primary-500 focus:ring-primary-500 focus:ring-offset-0 cursor-pointer shadow-sm drop-shadow-md"
                          />
                        </div>
                      </div>
          ...
          {/* Selection Modal for delete/clear */}
          {showSelectionModal && (
            <div className="fixed inset-0 bg-black/50 z-[200] flex items-center justify-center">
              <div className="w-full max-w-3xl bg-white dark:bg-slate-900 rounded-2xl p-6">
                <h3 className="text-lg font-black mb-4">Select products to delete</h3>
                <div className="max-h-96 overflow-y-auto space-y-3 mb-4">
                  {products.filter(p => p.is_published).map(p => (
                    <div key={p.id} className={`p-3 rounded-xl border ${selectedIds.includes(p.id) ? 'border-primary-500 bg-primary-50 dark:bg-primary-900/10' : 'border-slate-200 dark:border-slate-800'} flex items-center gap-3 cursor-pointer`} onClick={() => {
                      setSelectedIds(prev => prev.includes(p.id) ? prev.filter(id => id !== p.id) : [...prev, p.id]);
                    }}>
                      <input type="checkbox" checked={selectedIds.includes(p.id)} onChange={(e: React.ChangeEvent<HTMLInputElement>) => {
                        e.stopPropagation();
                        setSelectedIds(prev => e.target.checked ? [...prev, p.id] : prev.filter(id => id !== p.id));
                      }} onClick={(e) => e.stopPropagation()} className="w-5 h-5" />
                      <div className="flex-1">
                        <div className="font-bold">{p.name}</div>
                        <div className="text-xs text-slate-500">{p.shops?.name || 'Shop'}</div>
                      </div>
                      <div className="text-sm font-black">${new Intl.NumberFormat().format(p.price)}</div>
                    </div>
                  ))}
                </div>
                <div className="flex justify-end gap-3">
                  <button onClick={() => { setShowSelectionModal(false); setSelectedIds([]); }} className="px-4 py-2 rounded-xl border">Cancel</button>
                  ...
                  }} className="px-4 py-2 bg-red-500 text-white rounded-xl">{isDeleting ? 'Deleting...' : 'Delete Selected'}</button>
                </div>
              </div>
            </div>
          )}
